...
export function getReminderTypeText(type: ReminderType): string {
    if (type === ReminderType.NONE)
        return '无提醒';
    if (type === ReminderType.AT_TIME)
        return '准时';
    if (type === ReminderType.FIVE_MIN)
        return '5分钟前';
    if (type === ReminderType.TEN_MIN)
        return '10分钟前';
    if (type === ReminderType.THIRTY_MIN)
        return '30分钟前';
    if (type === ReminderType.ONE_HOUR)
        return '1小时前';
    if (type === ReminderType.ONE_DAY)
        return '1天前';
    return '无提醒';
}
/**
